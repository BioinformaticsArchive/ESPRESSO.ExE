...
#' @description Gets the number of cases and controls or subjects and the empirical and theoretical 
#' power under each model and prints a summary on the screen
#' @param scenario Scenario number
#' @param pheno.model Type of the outcome; 0 for binary and 2 for continuous
#' @param env1.model Model of the 1st enviromental explosure
#' @param env2.model Model of the 2nd enviromental explosure
...
#' @author Gaye A.
#'
get.critical.results.ExE <-
function(scenario=NULL, pheno.model=NULL,env1.model=NULL,env2.model=NULL,sample.sizes.required=NULL,
         empirical.power=NULL,modelled.power=NULL,mean.beta=NULL)
{
   ...
   if(env1.model==0){
 	   ...
 	   if(env1.model==1){
 	     e1.model <- "normally distributed"
 	   }else{
 	     e1.model <- "uniformly distributed"
 	   }
 	 }
   ...
   if(env2.model==0){
     ...
     if(env2.model==1){
       e2.model <- "normally distributed"
     }else{
       e2.model <- "uniformly distributed"
     }
   }
  ...
  if(pheno.model==0){
     ...
     estimated.effect <- 'NA'
					...
					cat("------\n")
					cat("  Outcome: binary \n")
					cat("  First environmental determinant:",e1.model,"\n")
					cat("  Second environmental determinant:",e2.model)
		   ...
		   crit.res <- c(e1.model,e2.model,numcases,numcontrols,round(empirical.power,2),round(modelled.power,2),
     ...
     estimated.OR <- 'NA'
          ...
          cat("  Second environmental determinant:",e2.model)
		   ...
		   crit.res <- c(e1.model,e2.model,numsubjects,round(empirical.power,2),round(modelled.power,2),estimated.OR,round(estimated.effect,2))
		   return(list(environment1.model=crit.res[1], environment2.model=crit.res[2],number.of.subjects.required=crit.res[3],
