...
#' @description Fits a conventional unconditional logistic regression model with a binary or 
#' continuous phentype as outcome and the genetic, environmental, interaction determinants as covariates.
...
function(pheno.model=NULL, observed.data=NULL)
{
  ...
  if(pheno.model == 0){
	   ...
	   mod.glm <- glm(phenotype ~ 1+environment1+environment2+interaction,
